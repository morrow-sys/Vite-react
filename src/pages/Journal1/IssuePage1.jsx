...
const IssuePage1 = () => {
  ...
  useEffect(() => {
    ...
    const fetchJournalName = async () => {
      try {
        ...
        if (!res.ok) throw new Error(`Ошибка сервера: ${res.status}`);
        const data = await res.json();
        // Главное название журнала — не переводится
        setJournalName(data.name || "Наука, новые технологии и инновации Кыргызстана");
      } catch (e) {
        setJournalName("Наука, новые технологии и инновации Кыргызстана");
      }
    };
  ...
  if (!articles.length) return <Typography align="center">{t("noArticles")}</Typography>;
  ...
  return (
    <Box sx={{ p: 6, maxWidth: 900, mx: "auto" }}>
      ...
      {/* Заголовок выпуска/архива — переводится через i18n */}
      <Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
        ...
        >
          {t("archiveTitle", { year, issueNumber, count: articles.length })}
        </Typography>
        <Box sx={{ flex: 1, borderBottom: "1px solid #ccc" }} />
      </Box>
